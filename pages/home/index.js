...
Page({
  ...
  onLoad: function (options) {
    let that = this;
    if (wx.getStorageSync('userIdInfo') && wx.getStorageSync('userIdInfo') != '') {
      let member_id = wx.getStorageSync('userIdInfo').id;
      that.setData({
        id: member_id,
        showCop:true
      })
      ...
      app.getLogin().then(function(res){
        if (res.statusCode == 200) {
          let member_id = res.data.info.id;
          that.setData({
            id: member_id,
            showCop: true
          })
  ...
  formSubmit:function(e){
    ...
    let param = {
      member_id: this.data.id ? this.data.id : wx.setStorageSync('id'),
